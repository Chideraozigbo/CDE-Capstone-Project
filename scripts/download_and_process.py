    ...
    for country in json_data:
        try:
            data = {
                ...
                'common_native_name': country['name']['nativeName']['eng']['common'],
                'currency_code': list(country['currencies'])[0],
                'currency_name': country['currencies'][list(country['currencies'])[0]]['name'],
                'currency_symbol': country['currencies'][list(country['currencies'])[0]]['symbol'],
            ...
            print(f"Error extracting data for country {country['name']['common']}: {e}")
    if country_data:
        try:
...
def extract_country_data(json_data):
    ...
    for country in json_data:
        try:
            ...
            common_native_name = country['name']['nativeName'].get(native_name_key, {}).get('common', '')
            
            data = {
                'Country_Name': country['name']['common'],
                'independence': country['independent'] if country['independent'] else '',
                'united_nation_members': country['unMember'] if country['unMember'] else '',
                'start_of_week': country['startOfWeek'] if country['startOfWeek'] else '',
                'official_name': country['name']['official'] if country['name']['official'] else '',
                'common_native_name': common_native_name,
                'currency_code': list(country['currencies'])[0] if country['currencies'] else '',
                'currency_name': country['currencies'][list(country['currencies'])[0]]['name'] if country['currencies'] else '',
                'currency_symbol': country['currencies'][list(country['currencies'])[0]]['symbol'] if country['currencies'] else '',
                'country_code': f"+{country['idd']['root']}{country['idd']['suffixes'][0]}",
                ...
                'languages': ', '.join(country['languages'].values()) if country['languages'] else '',
                'area': country['area'] if country['area'] else '' ,
                'population': country['population'] if country['population'] else '',
                'continents': ', '.join(country['continents']) if country['continents'] else ''
